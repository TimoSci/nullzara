defmodule UserphoenixWeb.UserLive.Form do
  ...
  defp save_user(socket, :new, user_params) do
    case Users.create_user_with_token(user_params) do
      {:ok, user} ->
        url = url(~p"/u/#{user.raw_token}")
        ...
        {:noreply,
         ...
         |> put_flash(
           :info,
           "User created. Access URL: #{url} — Recovery phrase: #{mnemonic}"
         )
         |> push_navigate(to: return_path(socket.assigns.return_to, user))}
